CREATE PROCEDURE placerEnchere @montantEnchere INT,
                               @noArticle INT,
                               @noUtilisateur INT,
                               @EnchereId INT OUTPUT,
...
-- Vérification de l'enchérisseur
    SELECT @EncherisseurId = no_utilisateur,
    ...
    SELECT TOP 1 @MaxEnchereUID = no_utilisateur,
    ...
    IF @EnchereInvalide <> 0
        BEGIN
            SET @errorMessage = CASE
                WHEN @EnchereInvalide = -1 THEN 'Pas de vendeur, pas de vente'
                WHEN @EnchereInvalide = -2 THEN 'Vente non active'
                WHEN @EnchereInvalide = -3 THEN 'Pas d''enchère possible pour le vendeur'
                WHEN @EnchereInvalide = -4 THEN 'Enchérisseur inconnu'
                WHEN @EnchereInvalide = -5 THEN 'Déjà meilleur enchérisseur'
                WHEN @EnchereInvalide = -6 THEN 'Pas assez de crédit ou enchère trop faible'
                WHEN @EnchereInvalide = -7 THEN 'Montant enchère trop faible'
                ELSE 'Erreur inconnue'
                END;
            RETURN @EnchereInvalide;
        END

-- Si aucune erreur, commencer la transaction
    BEGIN TRANSACTION;
